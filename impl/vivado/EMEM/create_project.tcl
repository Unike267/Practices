...
## Design: processor subsystem template, and (optionally) BoardTop and/or other additional sources (Mult_wfifos via wishbone is included)
if {[info exists ::env(DESIGN) ]} {
  set design $::env(DESIGN)
    if {$design == "mult"} {
      set fileset_design $mult/EMEM/neorv32_mult_wfifos_wishbone.vhd
    } elseif {$design == "multp-wfifos"} {
      set fileset_design $multp/EMEM/neorv32_multp_wfifos_wishbone.vhd
    } else {
      error "Specify a valid value for DESIGN \[$design\]: 'mult', 'multp-wfifos'"
    }
} else {
  error "Specify a value for DESIGN: 'mult', 'multp-wfifos'"
}
