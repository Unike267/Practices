...
## Design: processor subsystem template, and (optionally) BoardTop and/or other additional sources (Mult_wfifos via axi is included)
if {[info exists ::env(DESIGN) ]} {
  set design $::env(DESIGN)
    if {$design == "mult"} {
      set fileset_design $mult/slink/neorv32_mult_wfifos_slink.vhd
    } elseif {$design == "multp"} {
      set fileset_design $multp/slink/neorv32_multp_slink.vhd
    } elseif {$design == "multp-wfifos"} {
      set fileset_design $multp/slink/neorv32_multp_wfifos_slink.vhd
    } else {
      error "Specify a valid value for DESIGN \[$design\]: 'mult', 'multp', 'multp-wfifos'"
    }
} else {
  error "Specify a value for DESIGN: 'mult', 'multp', 'multp-wfifos'"
}
