...
if DESIGN == "mult":
    csv_file = csv_path / "tb_mult_wfifos_wishbone_throughput.csv"
elif DESIGN == "multp-wfifos":
    csv_file = csv_path / "tb_multp_wfifos_wishbone_throughput.csv"
else:
    print("The valid envvar values are: mult and multp-wfifos")
    exit()
...
for k in range(0,len(line)):
    if int(matrix[1][k]) == 87: # If the time information comes from line 99 is sent information
        sent.append(matrix[0][k]) # Save the sent time information
    elif int(matrix[1][k]) == 96: # If the time information comes from line 108 is received information
