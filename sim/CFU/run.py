...
def post_func(results):
    ...
    for key, item in report.tests.items():
        if key == "neorv32.tb_complex_mults_cfu.test": # Copy the output csv and the wave in vcd of tb_complex_mults to known path
            copyfile(
                str(Path(item.path) / "log.csv"),
                str(out_dir_csv / "tb_complex_mults_cfu.csv"),
            )
            copyfile(
                str(Path(item.path) / "ghdl" / "wave.vcd"),
                str(out_dir_wave / "wave_complex_mults_cfu.vcd"),
            )
...
NEORV32.add_source_files([
    ROOT / "../../rtl/mult/*.vhd",
    ROOT / "../../rtl/multp/*.vhd",
    ROOT / "../../rtl/mult/CFU/neorv32_mults_cfu.vhd",
    ROOT / "*.vhd",
